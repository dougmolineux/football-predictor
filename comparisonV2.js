...
const config = {
    neuronCount: 64,  // Increased from 10 to 64
    batchSize: 32,    // Increased batch size for more stable gradient updates
    epochs: 10,
...
async function createModel(inputs, outputs) {
    ...
    model.add(tf.layers.dropout({ rate: config.dropoutRate }));  // Add dropout layer
    ...
    const optimizer = tf.train.adam(config.learningRate);  // Adjusted learning rate
    model.compile({
        optimizer: optimizer,
